...
public class PetitionController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(CreatePetitionRequest request)
    {
    ...
    {
        PagedResult<PetitionDto> result = await _service.ListAsync(page, pageSize);
        return Ok(result);
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, UpdatePetitionRequest request)
    {
